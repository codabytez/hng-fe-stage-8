...
interface Token {
  type: "keyword" | "field" | "operator" | "string" | "number" | "punctuation" | "comment" | "plain";
  text: string;
}

const SQL_KEYWORDS = new Set(["SELECT", "FROM", "WHERE", "AND", "OR", "NOT", "IN", "LIKE", "BETWEEN", "IS", "NULL", "TRUE", "FALSE", "DATE", "MONTH", "YEAR", "NOW", "CURDATE", "YEARWEEK", "JSON_CONTAINS", "JSON_LENGTH", "REGEXP"]);
...
function tokenizeJSON(code: string): Token[] {
  ...
  while ((match = regex.exec(code)) !== null) {
    ...
    if (match[1]) {
      const isKey = code[match.index - 1] === "\n" || /^\s*$/.test(code.slice(0, match.index).split("\n").pop() ?? "");
      tokens.push({ type: isKey && code[match.index + match[0].length]?.trimStart().startsWith(":") ? "field" : "string", text: match[0] });
    } else if (match[2]) {
      tokens.push({ type: match[2] === "null" ? "operator" : "keyword", text: match[0] });
    } else if (match[3]) {
...
function tokenizeGraphQL(code: string): Token[] {
  const GQL_KEYWORDS = new Set(["query", "mutation", "subscription", "fragment", "on", "where"]);
  const tokens: Token[] = [];
  const parts = code.split(/(\s+|[{}()[\]:,]|"[^"]*"|#[^\n]*|\b\w+\b)/g).filter(Boolean);
  for (const part of parts) {
    if (part.startsWith("#")) tokens.push({ type: "comment", text: part });
    else if (part.startsWith('"')) tokens.push({ type: "string", text: part });
    else if (GQL_KEYWORDS.has(part)) tokens.push({ type: "keyword", text: part });
    else if (/^[{}()[\]:,]$/.test(part)) tokens.push({ type: "punctuation", text: part });
...
export const CodeBlock = React.memo(function CodeBlock({ code, language, className }: CodeBlockProps) {
  if (!code.trim()) {
    return (
      <pre className={cn("min-h-[200px] rounded-md border border-code-border bg-code-bg p-4 font-mono text-sm leading-6", className)}>
  ...
  const tokens = language === "sql" ? tokenizeSQL(code) : language === "json" ? tokenizeJSON(code) : tokenizeGraphQL(code);

  return (
    <pre className={cn("overflow-auto rounded-md border border-code-border bg-code-bg p-4 font-mono text-sm leading-6", className)}>
      <code>
        {tokens.map((token, i) => (
          token.type === "plain" ? token.text : (
            <span key={i} className={TYPE_CLASS[token.type]}>{token.text}</span>
          )
        ))}
      </code>
    </pre>
